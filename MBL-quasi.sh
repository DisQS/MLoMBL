...
echo "MBL-Q: making for L=" $size "with starting seed=" $seed "and" $config "samples"
...
a4=45678*$seed
batch=10
#path='/gpfs/home/p/phrczh/Qusp/MBLquasi'
...
for L in Llist:
    ...
    for b in blist:
        ...
        pre=a1+a2*L+int(a3*b)+a4*(1+(int(sys.argv[1])))
        np.random.seed(pre)
        ...
        h_z=[[b*np.cos(2*np.pi*beta*i+2*b*np.random.random()-b),i] for i in range(L)]
        # static and dynamic lists
        static=[["zz",J_nn],["xx",J_nn],["yy",J_nn]]
        field=[["z",h_z]]
        #print(static[0][1][0])
        # compute Hamiltonians
        H=hamiltonian(static,[],dtype=np.float64,basis=basis)+hamiltonian(field,[],dtype=np.float64,basis=basis,check_symm=False)
        #print(H)
        E_fermion=H.eigvalsh()
        #print(E_fermion[1])
        outfile=f"/L={L}-{1+(int(sys.argv[1]))}-{'%.1f' %b}.csv"
        #print(path)
        #print(outfile)
